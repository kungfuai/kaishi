...
def add_jpeg_compression(image, quality_level: int = 30):
...
def add_rotation(image, ccw_rotation_degrees: int = 90):
...
def add_stretching(image, w_percent_additional, h_percent_additional):
    ...
    newsize = (
        image.size[0] * int(1.0 + float(w_percent_additional) / 100),
        image.size[1] * int(1.0 + float(h_percent_additional) / 100),
    )

    return image.resize(newsize, resample=Image.BILINEAR)


def add_poisson_noise(image, param: float = 1.0, rescale: bool = True):
